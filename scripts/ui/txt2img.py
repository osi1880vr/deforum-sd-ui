	...
	with st.form("txt2img-inputs"):
		...
		with col1:
			...
			st.session_state["scale"] = st.slider("CFG (Classifier Free Guidance Scale):", min_value=1.0,
												  max_value=30.0, value=st.session_state['defaults'].txt2img.scale,
												  step=1e-1, format="%.1f",
												  help="How strongly the image should follow the prompt.")
			st.session_state["seed"] = st.text_input("Seed:", value=st.session_state['defaults'].txt2img.seed,
													 help=" The seed to use, if left blank a random seed will be generated.")
			st.session_state["iterations"] = st.slider("Iterations:", min_value=1, max_value=2048,
													   value=st.session_state['defaults'].txt2img.iterations, step=1)
			st.session_state["batch_size"] = st.slider("Batchsize:", min_value=1, max_value=50,
		...
		with col2:
			...
			with col3:
				...
				st.session_state["steps"] = st.number_input('Sample Steps',
															value=st.session_state['defaults'].txt2img.steps, step=1)
