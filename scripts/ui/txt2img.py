...
def layoutFunc():
    ...
    with st.form("txt2img-inputs"):
        ...
        with col1:
            st.session_state["pathmode"] = st.selectbox('Path Structure', ("subfolders", "root"), value=st.session_state['defaults'].general.default_path_mode, help="subfolders structure will create daily folders plus many subfolders, root will use your outdir as root", key='pathmode-txt2img')
        ...
        with col2:
            ...
            with col3:
                ...
                st.session_state["steps"] = st.number_input('Sample Steps', value=st.session_state['defaults'].txt2img.steps,step=1)
                ...
                with st.expander("Single Frame"):
                    st.session_state["ddim_eta"] = st.number_input('DDIM ETA', value=st.session_state['defaults'].txt2img.ddim_eta,step=1e-1,format="%.1f")
                    ...
                    st.session_state["grid_rows"] = st.number_input('Hight', value=st.session_state['defaults'].txt2img.grid_rows,step=1)
                    st.session_state["use_init"] = st.checkbox('Use Init', value=False)
                    st.session_state["strength"] = st.number_input('Strength', value=st.session_state['defaults'].txt2img.strength,step=1e-1,format="%.1f")
                    ...
                    st.session_state["init_image"] = st.text_input("Init Image:", value=st.session_state['defaults'].txt2img.init_image, help="The image to be used as init")
                    ...
                    st.session_state["mask_file"] = st.text_input("Init Image:", value=st.session_state['defaults'].txt2img.mask_file, help="The Mask to be used")
                    st.session_state["invert_mask"] = st.checkbox('Invert Mask', value=False)
                    st.session_state["mask_brightness_adjust"] = st.number_input('Brightness Adjust', value=st.session_state['defaults'].txt2img.mask_brightness_adjust,step=1e-1,format="%.1f", help="Adjust the brightness of the mask")
                    st.session_state["mask_contrast_adjust"] = st.number_input('Contrast Adjust', value=st.session_state['defaults'].txt2img.mask_contrast_adjust,step=1e-1,format="%.1f", help="Adjust the contrast of the mask")
                ...
                with st.expander("Advanced"):
                    st.session_state["separate_prompts"] = st.checkbox("Create Prompt Matrix.", value=st.session_state['defaults'].txt2img.separate_prompts,
                                                                       help="Separate multiple prompts using the `|` character, and get all combinations of them.")
                    st.session_state["normalize_prompt_weights"] = st.checkbox("Normalize Prompt Weights.",
                                                                               value=st.session_state['defaults'].txt2img.normalize_prompt_weights, help="Ensure the sum of all weights add up to 1.0")
                    st.session_state["save_individual_images"] = st.checkbox("Save individual images.",
                                                                             value=st.session_state['defaults'].txt2img.save_individual_images, help="Save each image generated before any filter or enhancement is applied.")
                    st.session_state["group_by_prompt"] = st.checkbox("Group results by prompt", value=st.session_state['defaults'].txt2img.group_by_prompt,
                                                                      help="Saves all the images with the same prompt into the same folder. When using a prompt matrix each prompt combination will have its own folder.")
                    st.session_state["write_info_files"] = st.checkbox("Write Info file", value=st.session_state['defaults'].txt2img.write_info_files,
                                                                       help="Save a file next to the image with informartion about the generation.")
                    st.session_state["save_as_jpg"] = st.checkbox("Save samples as jpg", value=st.session_state['defaults'].txt2img.save_as_jpg, help="Saves the images as jpg instead of png.")

                    if GFPGAN_available:
                        st.session_state["use_GFPGAN"] = st.checkbox("Use GFPGAN", value=st.session_state['defaults'].txt2img.use_GFPGAN, help="Uses the GFPGAN model to improve faces after the generation. This greatly improve the quality and consistency of faces but uses extra VRAM. Disable if you need the extra VRAM.")
                    else:
