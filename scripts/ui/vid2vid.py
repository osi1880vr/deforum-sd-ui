...
def layoutFunc():
	...
	with col1:
		...
		with st.expander("Basic Settings", expanded=True):
			...
			st.session_state["prompt"] = st.text_area("Input Text", "",
			...
			st.session_state["max_frames"] = st.slider("Max Frames:", min_value=1, max_value=2048,
													   value=st.session_state['defaults'].vid2vid.max_frames, step=1,
													   key='Max Frames-vid2vid')

			st.session_state["W"] = st.slider("Width:", min_value=64, max_value=2048,
											  value=st.session_state['defaults'].vid2vid.W, step=64,
											  key='Width-vid2vid')
			st.session_state["H"] = st.slider("Height:", min_value=64, max_value=2048,
											  value=st.session_state['defaults'].vid2vid.H, step=64,
											  key='Height-vid2vid')
			st.session_state["scale"] = st.slider("CFG (Classifier Free Guidance Scale):", min_value=1.0,
												  max_value=30.0, value=st.session_state['defaults'].vid2vid.scale,
												  step=1e-1, format="%.1f",
												  help="How strongly the image should follow the prompt.",
												  key='Scale-vid2vid')

			st.session_state["seed"] = st.text_input("Seed:", value=st.session_state['defaults'].vid2vid.seed,
													 help=" The seed to use, if left blank a random seed will be generated.",
													 key='Seed-vid2vid')
			st.session_state["steps"] = st.number_input('Sample Steps',
	...
	with col2:
		...
		with sequence_tab:
			...
			st.session_state["angle"] = st.text_input("Angle:", value=st.session_state['defaults'].vid2vid.angle,
													  key='angle-vid2vid')
			st.session_state["zoom"] = st.text_input("Zoom:", value=st.session_state['defaults'].vid2vid.zoom,
													 key='zoom-vid2vid')
			st.session_state["translation_x"] = st.text_input("X Translation:",
															  value=st.session_state['defaults'].vid2vid.translation_x,
															  key='translation_x-vid2vid')
			st.session_state["translation_y"] = st.text_input("Y Translation:",
															  value=st.session_state['defaults'].vid2vid.translation_y,
															  key='translation_y-vid2vid')
			st.session_state["translation_z"] = st.text_input("Z Translation:",
															  value=st.session_state['defaults'].vid2vid.translation_z,
															  key='translation_z-vid2vid')
			st.session_state["rotation_3d_x"] = st.text_input("X 3D Rotaion:",
															  value=st.session_state['defaults'].vid2vid.rotation_3d_x,
															  key='rotation_3d_x-vid2vid')
			st.session_state["rotation_3d_y"] = st.text_input("Y 3D Rotaion:",
															  value=st.session_state['defaults'].vid2vid.rotation_3d_y,
															  key='rotation_3d_y-vid2vid')
			st.session_state["rotation_3d_z"] = st.text_input("Z 3D Rotaion:",
															  value=st.session_state['defaults'].vid2vid.rotation_3d_z,
															  key='rotation_3d_z-vid2vid')
			st.session_state["noise_schedule"] = st.text_input("Noise Schedule:", value=st.session_state[
				'defaults'].vid2vid.noise_schedule, key='noise_schedule-vid2vid')
			st.session_state["strength_schedule"] = st.text_input("Strength Schedule:", value=st.session_state[
				'defaults'].vid2vid.strength_schedule, key='strength_schedule-vid2vid')
			st.session_state["contrast_schedule"] = st.text_input("Contrast Schedule:", value=st.session_state[
		...
		with flip_sequence_tab:
			...
			st.session_state["perspective_flip_theta"] = st.text_input("Flip Theta:", value=st.session_state[
				'defaults'].vid2vid.perspective_flip_theta, key='perspective_flip_theta-vid2vid')
			st.session_state["perspective_flip_phi"] = st.text_input("Flip Phi:", value=st.session_state[
				'defaults'].vid2vid.perspective_flip_phi, key='perspective_flip_phi-vid2vid')
			st.session_state["perspective_flip_gamma"] = st.text_input("Flip Gamma:", value=st.session_state[
				'defaults'].vid2vid.perspective_flip_gamma, key='perspective_flip_gamma-vid2vid')
			st.session_state["perspective_flip_fv"] = st.text_input("Flip FV:", value=st.session_state[
	...
	with col3:
		...
		with st.expander("Animation", expanded=True):
			...
			if videoinit != None:
				...
				st.session_state["video_init_path"] = st.text_input("Video Init Path:",
																	value=st.session_state["init_path"],
																	help="Input Video Path",
																	key='video_init_path-vid2vid')
			else:
				st.session_state["video_init_path"] = st.text_input("Video Init Path:", value=st.session_state[
			...
			st.session_state["near_plane"] = st.number_input('Near Plane',
															 value=st.session_state['defaults'].vid2vid.near_plane,
															 step=1, key='near_plane-vid2vid')
			st.session_state["far_plane"] = st.number_input('Far Plane',
															value=st.session_state['defaults'].vid2vid.far_plane,
															step=1, key='far_plane-vid2vid')
			st.session_state["fov"] = st.number_input('FOV', value=st.session_state['defaults'].vid2vid.fov, step=1,
													  key='fov-vid2vid')
			...
			st.session_state["extract_nth_frame"] = st.number_input('Extract Nth Frame', value=st.session_state[
			...
			st.session_state["interpolate_x_frames"] = st.number_input('Number Frames to Interpolate',
			...
			st.session_state["resume_timestring"] = st.text_input("Resume Timestring:", value=st.session_state[
		...
		with st.expander("Single Frame"):
			...
			st.session_state["save_settings"] = st.checkbox('Save Settings', value=False,
			...
			st.session_state["grid_rows"] = st.number_input('Hight',
			...
			st.session_state["init_image"] = st.text_input("Init Image:",
			...
			st.session_state["mask_brightness_adjust"] = st.number_input('Brightness Adjust', value=st.session_state[
				'defaults'].vid2vid.mask_brightness_adjust, step=1e-1, format="%.1f",
																		 help="Adjust the brightness of the mask",
																		 key='mask_brightness_adjust-vid2vid')
			st.session_state["mask_contrast_adjust"] = st.number_input('Contrast Adjust', value=st.session_state[
