...
class PluginInfo():
    plugname = "txt2vid2"
    description = "Text to Video HLKY"
    isTab = False
    displayPriority = 1
...
def txt2vid(
    ...
    st.session_state["pipe"] = StableDiffusionPipeline.from_pretrained(
        ...
        use_local_file=True,
        use_auth_token=True,
        torch_dtype=None,#torch.float16 if st.session_state['defaults'].general.use_float16 else None,
        revision=None#"fp16" if not st.session_state['defaults'].general.no_half else None
    )
    st.session_state["pipe"].unet.to(torch_device)
    st.session_state["pipe"].vae.to(torch_device)
    st.session_state["pipe"].text_encoder.to(torch_device)
    ...
    if generate_button:
        ...
        try:
            ...
            video, seed, info, stats = txt2vid(prompts=prompt, gpu=st.session_state["defaults"].general.gpu,
                                               ...
                                               height=height, weights_path=st.session_state['defaults'].general.ckpt, scheduler=scheduler_name,
